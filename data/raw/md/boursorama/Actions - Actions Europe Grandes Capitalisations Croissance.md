---
grande_classe: "Actions"
type_actif: "Actions Europe Grandes Capitalisations Croissance"
slug: actions--actions-europe-grandes-capitalisations-croissance
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 6
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU0154234636"
    nom: "BGF-EUROPEAN SPEC SIT-A2E"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "-3,60%"
    frais_actif_b: "1,82%"
    perf_nette_c: "-5,42%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,53%"
    perf_finale: "-6,13%"
    retrocessions: "0,75%"
  - isin: "LU0099161993"
    nom: "CARMIGNAC GRANDE EUROA C"
    societe: "CARMIGNAC GESTION LUXEMBOURG"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "0,97%"
    frais_actif_b: "1,82%"
    perf_nette_c: "-0,85%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,56%"
    perf_finale: "-1,60%"
    retrocessions: "0,75%"
  - isin: "FR0000295230"
    nom: "COMGEST RENAISSANCE EUROPE C"
    societe: "COMGEST SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "-5,43%"
    frais_actif_b: "1,87%"
    perf_nette_c: "-7,30%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,57%"
    perf_finale: "-8,00%"
    retrocessions: "0,50%"
  - isin: "FR0010836163"
    nom: "CPR SILVER AGE P A/I"
    societe: "CPR ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "10,41%"
    frais_actif_b: "1,77%"
    perf_nette_c: "8,64%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,58%"
    perf_finale: "7,83%"
    retrocessions: "0,72%"
  - isin: "FR0010158048"
    nom: "DORVAL DRIVERS EUROPE R"
    societe: "DORVAL ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "8,79%"
    frais_actif_b: "2,01%"
    perf_nette_c: "6,78%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,81%"
    perf_finale: "5,98%"
    retrocessions: "1,00%"
  - isin: "FR0010321828"
    nom: "ECHIQUIER MAJ SRI GROWT EURP A"
    societe: "FINANCIERE DE L ECHIQUIER"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "-2,14%"
    frais_actif_b: "2,23%"
    perf_nette_c: "-4,37%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,95%"
    perf_finale: "-5,09%"
    retrocessions: "0,96%"
---

# Actions — Actions Europe Grandes Capitalisations Croissance

6 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU0154234636` | BGF-EUROPEAN SPEC SIT-A2E | BLACKROCK (LUXEMBOURG) SA | 2,53% | -6,13% | 0,75% | Article 8 |
| `LU0099161993` | CARMIGNAC GRANDE EUROA C | CARMIGNAC GESTION LUXEMBOURG | 2,56% | -1,60% | 0,75% | Article 9 |
| `FR0000295230` | COMGEST RENAISSANCE EUROPE C | COMGEST SA | 2,57% | -8,00% | 0,50% | Article 8 |
| `FR0010836163` | CPR SILVER AGE P A/I | CPR ASSET MANAGEMENT | 2,58% | 7,83% | 0,72% | Article 8 |
| `FR0010158048` | DORVAL DRIVERS EUROPE R | DORVAL ASSET MANAGEMENT | 2,81% | 5,98% | 1,00% | Article 8 |
| `FR0010321828` | ECHIQUIER MAJ SRI GROWT EURP A | FINANCIERE DE L ECHIQUIER | 2,95% | -5,09% | 0,96% | Article 8 |

---
## BGF-EUROPEAN SPEC SIT-A2E

`LU0154234636` · BLACKROCK (LUXEMBOURG) SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0154234636` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,82% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,53%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -3,60% |
| Nette de l'unité de compte (C = A − B) | -5,42% |
| **Finale (A − E)** | **-6,13%** |

---

## CARMIGNAC GRANDE EUROA C

`LU0099161993` · CARMIGNAC GESTION LUXEMBOURG · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU0099161993` |
| Société de gestion | CARMIGNAC GESTION LUXEMBOURG |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,82% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,56%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 0,97% |
| Nette de l'unité de compte (C = A − B) | -0,85% |
| **Finale (A − E)** | **-1,60%** |

---

## COMGEST RENAISSANCE EUROPE C

`FR0000295230` · COMGEST SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0000295230` |
| Société de gestion | COMGEST SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,87% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,57%** |
| Taux de rétrocession de commissionnement | 0,50% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -5,43% |
| Nette de l'unité de compte (C = A − B) | -7,30% |
| **Finale (A − E)** | **-8,00%** |

---

## CPR SILVER AGE P A/I

`FR0010836163` · CPR ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010836163` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,77% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,58%** |
| Taux de rétrocession de commissionnement | 0,72% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 10,41% |
| Nette de l'unité de compte (C = A − B) | 8,64% |
| **Finale (A − E)** | **7,83%** |

---

## DORVAL DRIVERS EUROPE R

`FR0010158048` · DORVAL ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010158048` |
| Société de gestion | DORVAL ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,01% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,81%** |
| Taux de rétrocession de commissionnement | 1,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 8,79% |
| Nette de l'unité de compte (C = A − B) | 6,78% |
| **Finale (A − E)** | **5,98%** |

---

## ECHIQUIER MAJ SRI GROWT EURP A

`FR0010321828` · FINANCIERE DE L ECHIQUIER · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010321828` |
| Société de gestion | FINANCIERE DE L ECHIQUIER |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Europe Grandes Capitalisations Croissance (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,23% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,95%** |
| Taux de rétrocession de commissionnement | 0,96% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -2,14% |
| Nette de l'unité de compte (C = A − B) | -4,37% |
| **Finale (A − E)** | **-5,09%** |
