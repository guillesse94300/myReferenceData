---
grande_classe: "Actions"
type_actif: "Secteur Technologies"
slug: actions--secteur-technologies
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 14
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU1861217674"
    nom: "BGF FINTECH A2 EUR HEDGED"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "12,04%"
    frais_actif_b: "0,00%"
    perf_nette_c: "12,04%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "0,84%"
    perf_finale: "11,20%"
    retrocessions: "0,75%"
  - isin: "LU0171310443"
    nom: "BGF WORLD TECHNOLOGY FD A2"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "6,21%"
    frais_actif_b: "1,81%"
    perf_nette_c: "4,40%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,59%"
    perf_finale: "3,61%"
    retrocessions: "0,75%"
  - isin: "LU0823421333"
    nom: "BNPP FDS DISRUPTIVE TECHNOLO C"
    societe: "BNP PARIBAS ASSET MNGT LUX SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "10,46%"
    frais_actif_b: "1,96%"
    perf_nette_c: "8,50%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,77%"
    perf_finale: "7,69%"
    retrocessions: "0,82%"
  - isin: "FR001400RWO8"
    nom: "BOURSO TECH"
    societe: "SG INVT SOLUTIONS FRANCE"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,59%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,00%"
  - isin: "LU2809794220"
    nom: "CARMIGNAC PORTFOLIO TECH SOL A"
    societe: "CARMIGNAC GESTION LUXEMBOURG"
    nature: "SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "1,80%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,75%"
  - isin: "LU2860962559"
    nom: "CPR INV ARTIFICIAL INTELLIG A"
    societe: "CPR ASSET MANAGEMENT"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "2,03%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,80%"
  - isin: "LU1244893696"
    nom: "EDR FUND BIG DATA A"
    societe: "EDR ASSET MANAGEMENT LUX SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "7,32%"
    frais_actif_b: "2,07%"
    perf_nette_c: "5,25%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,86%"
    perf_finale: "4,46%"
    retrocessions: "0,85%"
  - isin: "FR0013488244"
    nom: "EDR SICAV TECH FOR TOMOROW A"
    societe: "EDMOND ROTHSCHILD ASSET MNGT"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "15,95%"
    frais_actif_b: "1,96%"
    perf_nette_c: "13,99%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,81%"
    perf_finale: "13,13%"
    retrocessions: "0,98%"
  - isin: "LU0115773425"
    nom: "FF GLOBAL TECHNOLOGY FUND"
    societe: "FIL INVESTMENT MANAGEMENT LUX"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "11,79%"
    frais_actif_b: "2,64%"
    perf_nette_c: "9,15%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "3,46%"
    perf_finale: "8,33%"
    retrocessions: "0,75%"
  - isin: "LU1951200481"
    nom: "MIROVA THEM AIROBOT RC EUR"
    societe: "NATIXIS INVEST MANAGERS INTL"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "5,98%"
    frais_actif_b: "2,05%"
    perf_nette_c: "3,93%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,83%"
    perf_finale: "3,15%"
    retrocessions: "0,88%"
  - isin: "LU0340554913"
    nom: "PICTET DIGITAL P EUR CAP"
    societe: "PICTET ASSET MANAGEMENT EUROPE"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "6,71%"
    frais_actif_b: "1,99%"
    perf_nette_c: "4,72%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,78%"
    perf_finale: "3,94%"
    retrocessions: "0,80%"
  - isin: "FR0000431538"
    nom: "SG ACTIONS US TECHNO C"
    societe: "SOCIETE GENERALE GESTION"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "8,31%"
    frais_actif_b: "2,16%"
    perf_nette_c: "6,15%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,96%"
    perf_finale: "5,35%"
    retrocessions: "1,00%"
  - isin: "LU2181906426"
    nom: "SYCOMORE SUSTAINABLE TECH RC"
    societe: "SYCOMORE ASSET MANAGEMENT"
    nature: "SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "15,17%"
    frais_actif_b: "1,95%"
    perf_nette_c: "13,22%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,80%"
    perf_finale: "12,37%"
    retrocessions: "0,95%"
  - isin: "LU0109392836"
    nom: "TEMPLETON FRANKLIN TECHNO"
    societe: "FRANKLIN TEMPLET INTL SVCES SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "7,21%"
    frais_actif_b: "1,78%"
    perf_nette_c: "5,43%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,57%"
    perf_finale: "4,64%"
    retrocessions: "0,70%"
---

# Actions — Secteur Technologies

14 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU1861217674` | BGF FINTECH A2 EUR HEDGED | BLACKROCK (LUXEMBOURG) SA | 0,84% | 11,20% | 0,75% | non renseignée |
| `LU0171310443` | BGF WORLD TECHNOLOGY FD A2 | BLACKROCK (LUXEMBOURG) SA | 2,59% | 3,61% | 0,75% | Article 8 |
| `LU0823421333` | BNPP FDS DISRUPTIVE TECHNOLO C | BNP PARIBAS ASSET MNGT LUX SA | 2,77% | 7,69% | 0,82% | Article 8 |
| `FR001400RWO8` | BOURSO TECH | SG INVT SOLUTIONS FRANCE | n.c. | n.c. | 0,00% | Article 8 |
| `LU2809794220` | CARMIGNAC PORTFOLIO TECH SOL A | CARMIGNAC GESTION LUXEMBOURG | n.c. | n.c. | 0,75% | Article 9 |
| `LU2860962559` | CPR INV ARTIFICIAL INTELLIG A | CPR ASSET MANAGEMENT | n.c. | n.c. | 0,80% | Article 8 |
| `LU1244893696` | EDR FUND BIG DATA A | EDR ASSET MANAGEMENT LUX SA | 2,86% | 4,46% | 0,85% | Article 8 |
| `FR0013488244` | EDR SICAV TECH FOR TOMOROW A | EDMOND ROTHSCHILD ASSET MNGT | 2,81% | 13,13% | 0,98% | Article 9 |
| `LU0115773425` | FF GLOBAL TECHNOLOGY FUND | FIL INVESTMENT MANAGEMENT LUX | 3,46% | 8,33% | 0,75% | Article 8 |
| `LU1951200481` | MIROVA THEM AIROBOT RC EUR | NATIXIS INVEST MANAGERS INTL | 2,83% | 3,15% | 0,88% | Article 8 |
| `LU0340554913` | PICTET DIGITAL P EUR CAP | PICTET ASSET MANAGEMENT EUROPE | 2,78% | 3,94% | 0,80% | Article 8 |
| `FR0000431538` | SG ACTIONS US TECHNO C | SOCIETE GENERALE GESTION | 2,96% | 5,35% | 1,00% | Article 8 |
| `LU2181906426` | SYCOMORE SUSTAINABLE TECH RC | SYCOMORE ASSET MANAGEMENT | 2,80% | 12,37% | 0,95% | Article 9 |
| `LU0109392836` | TEMPLETON FRANKLIN TECHNO | FRANKLIN TEMPLET INTL SVCES SA | 2,57% | 4,64% | 0,70% | Article 8 |

---
## BGF FINTECH A2 EUR HEDGED

`LU1861217674` · BLACKROCK (LUXEMBOURG) SA · SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `LU1861217674` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,00% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **0,84%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 12,04% |
| Nette de l'unité de compte (C = A − B) | 12,04% |
| **Finale (A − E)** | **11,20%** |

---

## BGF WORLD TECHNOLOGY FD A2

`LU0171310443` · BLACKROCK (LUXEMBOURG) SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0171310443` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,81% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,59%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 6,21% |
| Nette de l'unité de compte (C = A − B) | 4,40% |
| **Finale (A − E)** | **3,61%** |

---

## BNPP FDS DISRUPTIVE TECHNOLO C

`LU0823421333` · BNP PARIBAS ASSET MNGT LUX SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0823421333` |
| Société de gestion | BNP PARIBAS ASSET MNGT LUX SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,96% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,77%** |
| Taux de rétrocession de commissionnement | 0,82% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 10,46% |
| Nette de l'unité de compte (C = A − B) | 8,50% |
| **Finale (A − E)** | **7,69%** |

---

## BOURSO TECH

`FR001400RWO8` · SG INVT SOLUTIONS FRANCE · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR001400RWO8` |
| Société de gestion | SG INVT SOLUTIONS FRANCE |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,59% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## CARMIGNAC PORTFOLIO TECH SOL A

`LU2809794220` · CARMIGNAC GESTION LUXEMBOURG · SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU2809794220` |
| Société de gestion | CARMIGNAC GESTION LUXEMBOURG |
| Nature juridique | SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,80% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## CPR INV ARTIFICIAL INTELLIG A

`LU2860962559` · CPR ASSET MANAGEMENT · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU2860962559` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,03% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,80% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## EDR FUND BIG DATA A

`LU1244893696` · EDR ASSET MANAGEMENT LUX SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1244893696` |
| Société de gestion | EDR ASSET MANAGEMENT LUX SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,07% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,86%** |
| Taux de rétrocession de commissionnement | 0,85% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 7,32% |
| Nette de l'unité de compte (C = A − B) | 5,25% |
| **Finale (A − E)** | **4,46%** |

---

## EDR SICAV TECH FOR TOMOROW A

`FR0013488244` · EDMOND ROTHSCHILD ASSET MNGT · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `FR0013488244` |
| Société de gestion | EDMOND ROTHSCHILD ASSET MNGT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,96% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,81%** |
| Taux de rétrocession de commissionnement | 0,98% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 15,95% |
| Nette de l'unité de compte (C = A − B) | 13,99% |
| **Finale (A − E)** | **13,13%** |

---

## FF GLOBAL TECHNOLOGY FUND

`LU0115773425` · FIL INVESTMENT MANAGEMENT LUX · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0115773425` |
| Société de gestion | FIL INVESTMENT MANAGEMENT LUX |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,64% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **3,46%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 11,79% |
| Nette de l'unité de compte (C = A − B) | 9,15% |
| **Finale (A − E)** | **8,33%** |

---

## MIROVA THEM AIROBOT RC EUR

`LU1951200481` · NATIXIS INVEST MANAGERS INTL · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1951200481` |
| Société de gestion | NATIXIS INVEST MANAGERS INTL |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,05% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,83%** |
| Taux de rétrocession de commissionnement | 0,88% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 5,98% |
| Nette de l'unité de compte (C = A − B) | 3,93% |
| **Finale (A − E)** | **3,15%** |

---

## PICTET DIGITAL P EUR CAP

`LU0340554913` · PICTET ASSET MANAGEMENT EUROPE · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0340554913` |
| Société de gestion | PICTET ASSET MANAGEMENT EUROPE |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,99% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,78%** |
| Taux de rétrocession de commissionnement | 0,80% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 6,71% |
| Nette de l'unité de compte (C = A − B) | 4,72% |
| **Finale (A − E)** | **3,94%** |

---

## SG ACTIONS US TECHNO C

`FR0000431538` · SOCIETE GENERALE GESTION · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0000431538` |
| Société de gestion | SOCIETE GENERALE GESTION |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,16% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,96%** |
| Taux de rétrocession de commissionnement | 1,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 8,31% |
| Nette de l'unité de compte (C = A − B) | 6,15% |
| **Finale (A − E)** | **5,35%** |

---

## SYCOMORE SUSTAINABLE TECH RC

`LU2181906426` · SYCOMORE ASSET MANAGEMENT · SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU2181906426` |
| Société de gestion | SYCOMORE ASSET MANAGEMENT |
| Nature juridique | SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,95% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,80%** |
| Taux de rétrocession de commissionnement | 0,95% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 15,17% |
| Nette de l'unité de compte (C = A − B) | 13,22% |
| **Finale (A − E)** | **12,37%** |

---

## TEMPLETON FRANKLIN TECHNO

`LU0109392836` · FRANKLIN TEMPLET INTL SVCES SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0109392836` |
| Société de gestion | FRANKLIN TEMPLET INTL SVCES SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Technologies (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,78% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,57%** |
| Taux de rétrocession de commissionnement | 0,70% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 7,21% |
| Nette de l'unité de compte (C = A − B) | 5,43% |
| **Finale (A − E)** | **4,64%** |
