---
grande_classe: "Actions"
type_actif: "Actions Zone euro"
slug: actions--actions-zone-euro
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 7
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU1883303635"
    nom: "AMUNDI FDS EUROLAND EQUITY A"
    societe: "AMUNDI LUXEMBOURG SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "17,79%"
    frais_actif_b: "1,78%"
    perf_nette_c: "16,01%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,65%"
    perf_finale: "15,14%"
    retrocessions: "0,75%"
  - isin: "BE0058182792"
    nom: "DPAM INV B EQ EUROLAND B CAP"
    societe: "DEGROOF PETERCAM ASSET MNGT SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "13,03%"
    frais_actif_b: "1,76%"
    perf_nette_c: "11,27%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,59%"
    perf_finale: "10,44%"
    retrocessions: "0,80%"
  - isin: "LU2219351876"
    nom: "FF FD SUS EUROZONE EQ FD A"
    societe: "FIL INVESTMENT MANAGEMENT LUX"
    nature: "SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "15,09%"
    frais_actif_b: "1,92%"
    perf_nette_c: "13,17%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,77%"
    perf_finale: "12,32%"
    retrocessions: "0,90%"
  - isin: "LU2985305460"
    nom: "IMGP EURO SELECT FUND C"
    societe: "IM GLOBAL PARTNER ASSET MGMT"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "2,39%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,70%"
  - isin: "FR0013275112"
    nom: "OFI INVEST ESG EURO EQ ACT R"
    societe: "OFI INVEST ASSET MANAGEMENT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "17,34%"
    frais_actif_b: "1,82%"
    perf_nette_c: "15,52%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,69%"
    perf_finale: "14,66%"
    retrocessions: "0,88%"
  - isin: "LU0503372608"
    nom: "ROBECO EURO SDG CREDITS D"
    societe: "ROBECO INSTITUTIONAL ASSET MNG"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "3,23%"
    frais_actif_b: "0,92%"
    perf_nette_c: "2,31%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,69%"
    perf_finale: "1,54%"
    retrocessions: "0,35%"
  - isin: "FR0000448847"
    nom: "SG ACTIONS EURO SMALL C IC"
    societe: "SG INVT SOLUTIONS FRANCE"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "22,01%"
    frais_actif_b: "1,53%"
    perf_nette_c: "20,48%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,43%"
    perf_finale: "19,57%"
    retrocessions: "0,72%"
---

# Actions — Actions Zone euro

7 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU1883303635` | AMUNDI FDS EUROLAND EQUITY A | AMUNDI LUXEMBOURG SA | 2,65% | 15,14% | 0,75% | Article 8 |
| `BE0058182792` | DPAM INV B EQ EUROLAND B CAP | DEGROOF PETERCAM ASSET MNGT SA | 2,59% | 10,44% | 0,80% | Article 8 |
| `LU2219351876` | FF FD SUS EUROZONE EQ FD A | FIL INVESTMENT MANAGEMENT LUX | 2,77% | 12,32% | 0,90% | Article 9 |
| `LU2985305460` | IMGP EURO SELECT FUND C | IM GLOBAL PARTNER ASSET MGMT | n.c. | n.c. | 0,70% | Article 8 |
| `FR0013275112` | OFI INVEST ESG EURO EQ ACT R | OFI INVEST ASSET MANAGEMENT | 2,69% | 14,66% | 0,88% | Article 8 |
| `LU0503372608` | ROBECO EURO SDG CREDITS D | ROBECO INSTITUTIONAL ASSET MNG | 1,69% | 1,54% | 0,35% | Article 9 |
| `FR0000448847` | SG ACTIONS EURO SMALL C IC | SG INVT SOLUTIONS FRANCE | 2,43% | 19,57% | 0,72% | Article 8 |

---
## AMUNDI FDS EUROLAND EQUITY A

`LU1883303635` · AMUNDI LUXEMBOURG SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1883303635` |
| Société de gestion | AMUNDI LUXEMBOURG SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,78% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,65%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 17,79% |
| Nette de l'unité de compte (C = A − B) | 16,01% |
| **Finale (A − E)** | **15,14%** |

---

## DPAM INV B EQ EUROLAND B CAP

`BE0058182792` · DEGROOF PETERCAM ASSET MNGT SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `BE0058182792` |
| Société de gestion | DEGROOF PETERCAM ASSET MNGT SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,76% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,59%** |
| Taux de rétrocession de commissionnement | 0,80% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 13,03% |
| Nette de l'unité de compte (C = A − B) | 11,27% |
| **Finale (A − E)** | **10,44%** |

---

## FF FD SUS EUROZONE EQ FD A

`LU2219351876` · FIL INVESTMENT MANAGEMENT LUX · SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU2219351876` |
| Société de gestion | FIL INVESTMENT MANAGEMENT LUX |
| Nature juridique | SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,92% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,77%** |
| Taux de rétrocession de commissionnement | 0,90% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 15,09% |
| Nette de l'unité de compte (C = A − B) | 13,17% |
| **Finale (A − E)** | **12,32%** |

---

## IMGP EURO SELECT FUND C

`LU2985305460` · IM GLOBAL PARTNER ASSET MGMT · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU2985305460` |
| Société de gestion | IM GLOBAL PARTNER ASSET MGMT |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,39% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,70% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## OFI INVEST ESG EURO EQ ACT R

`FR0013275112` · OFI INVEST ASSET MANAGEMENT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0013275112` |
| Société de gestion | OFI INVEST ASSET MANAGEMENT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,82% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,69%** |
| Taux de rétrocession de commissionnement | 0,88% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 17,34% |
| Nette de l'unité de compte (C = A − B) | 15,52% |
| **Finale (A − E)** | **14,66%** |

---

## ROBECO EURO SDG CREDITS D

`LU0503372608` · ROBECO INSTITUTIONAL ASSET MNG · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU0503372608` |
| Société de gestion | ROBECO INSTITUTIONAL ASSET MNG |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,92% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,69%** |
| Taux de rétrocession de commissionnement | 0,35% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 3,23% |
| Nette de l'unité de compte (C = A − B) | 2,31% |
| **Finale (A − E)** | **1,54%** |

---

## SG ACTIONS EURO SMALL C IC

`FR0000448847` · SG INVT SOLUTIONS FRANCE · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0000448847` |
| Société de gestion | SG INVT SOLUTIONS FRANCE |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Actions Zone euro (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,53% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,43%** |
| Taux de rétrocession de commissionnement | 0,72% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 22,01% |
| Nette de l'unité de compte (C = A − B) | 20,48% |
| **Finale (A − E)** | **19,57%** |
