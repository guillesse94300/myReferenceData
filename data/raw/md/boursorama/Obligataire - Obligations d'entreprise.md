---
grande_classe: "Obligataire"
type_actif: "Obligations d'entreprise"
slug: obligataire--obligations-d-entreprise
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 7
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "FR001400C9Z9"
    nom: "BFT CREDIT EUR OBLI DUR P C"
    societe: "CPR ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,72%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,30%"
  - isin: "FR0010172767"
    nom: "EDR EURO SUSTAINABLE CREDIT C"
    societe: "EDMOND ROTHSCHILD ASSET MNGT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "3,89%"
    frais_actif_b: "1,11%"
    perf_nette_c: "2,78%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,88%"
    perf_finale: "2,01%"
    retrocessions: "0,54%"
  - isin: "FR0013460920"
    nom: "EDR SHORT DURATION CREDIT A"
    societe: "EDMOND ROTHSCHILD ASSET MNGT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "5,22%"
    frais_actif_b: "0,85%"
    perf_nette_c: "4,37%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,63%"
    perf_finale: "3,59%"
    retrocessions: "0,35%"
  - isin: "LU1752460292"
    nom: "ODDO BHF CREDIT OPPORT CR EUR"
    societe: "ODDO BHF ASSET MANAGEMENT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "3,64%"
    frais_actif_b: "1,12%"
    perf_nette_c: "2,52%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,89%"
    perf_finale: "1,75%"
    retrocessions: "0,50%"
  - isin: "LU1486845537"
    nom: "ODDO BHF EURO CREDIT SHORT DUR"
    societe: "ODDO BHF ASSET MANAGEMENT"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "4,56%"
    frais_actif_b: "0,92%"
    perf_nette_c: "3,64%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,70%"
    perf_finale: "2,87%"
    retrocessions: "0,20%"
  - isin: "LU0145975222"
    nom: "ODDO BHF SUST EURO CORP BD CR"
    societe: "ODDO BHF ASSET MANAGEMENT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "3,39%"
    frais_actif_b: "1,02%"
    perf_nette_c: "2,37%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,79%"
    perf_finale: "1,60%"
    retrocessions: "0,55%"
  - isin: "LU0113257694"
    nom: "SCHRODER EURO CORPORATE"
    societe: "SCHRODER INVESTMENT MGT EUR SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "4,47%"
    frais_actif_b: "1,03%"
    perf_nette_c: "3,44%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,81%"
    perf_finale: "2,66%"
    retrocessions: "0,38%"
---

# Obligataire — Obligations d'entreprise

7 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `FR001400C9Z9` | BFT CREDIT EUR OBLI DUR P C | CPR ASSET MANAGEMENT | n.c. | n.c. | 0,30% | Article 8 |
| `FR0010172767` | EDR EURO SUSTAINABLE CREDIT C | EDMOND ROTHSCHILD ASSET MNGT | 1,88% | 2,01% | 0,54% | Article 8 |
| `FR0013460920` | EDR SHORT DURATION CREDIT A | EDMOND ROTHSCHILD ASSET MNGT | 1,63% | 3,59% | 0,35% | Article 8 |
| `LU1752460292` | ODDO BHF CREDIT OPPORT CR EUR | ODDO BHF ASSET MANAGEMENT | 1,89% | 1,75% | 0,50% | Article 8 |
| `LU1486845537` | ODDO BHF EURO CREDIT SHORT DUR | ODDO BHF ASSET MANAGEMENT | 1,70% | 2,87% | 0,20% | Article 8 |
| `LU0145975222` | ODDO BHF SUST EURO CORP BD CR | ODDO BHF ASSET MANAGEMENT | 1,79% | 1,60% | 0,55% | Article 8 |
| `LU0113257694` | SCHRODER EURO CORPORATE | SCHRODER INVESTMENT MGT EUR SA | 1,81% | 2,66% | 0,38% | Article 8 |

---
## BFT CREDIT EUR OBLI DUR P C

`FR001400C9Z9` · CPR ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR001400C9Z9` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,72% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,30% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## EDR EURO SUSTAINABLE CREDIT C

`FR0010172767` · EDMOND ROTHSCHILD ASSET MNGT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010172767` |
| Société de gestion | EDMOND ROTHSCHILD ASSET MNGT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,11% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,88%** |
| Taux de rétrocession de commissionnement | 0,54% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 3,89% |
| Nette de l'unité de compte (C = A − B) | 2,78% |
| **Finale (A − E)** | **2,01%** |

---

## EDR SHORT DURATION CREDIT A

`FR0013460920` · EDMOND ROTHSCHILD ASSET MNGT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0013460920` |
| Société de gestion | EDMOND ROTHSCHILD ASSET MNGT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,85% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,63%** |
| Taux de rétrocession de commissionnement | 0,35% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 5,22% |
| Nette de l'unité de compte (C = A − B) | 4,37% |
| **Finale (A − E)** | **3,59%** |

---

## ODDO BHF CREDIT OPPORT CR EUR

`LU1752460292` · ODDO BHF ASSET MANAGEMENT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1752460292` |
| Société de gestion | ODDO BHF ASSET MANAGEMENT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,12% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,89%** |
| Taux de rétrocession de commissionnement | 0,50% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 3,64% |
| Nette de l'unité de compte (C = A − B) | 2,52% |
| **Finale (A − E)** | **1,75%** |

---

## ODDO BHF EURO CREDIT SHORT DUR

`LU1486845537` · ODDO BHF ASSET MANAGEMENT · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1486845537` |
| Société de gestion | ODDO BHF ASSET MANAGEMENT |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,92% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,70%** |
| Taux de rétrocession de commissionnement | 0,20% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 4,56% |
| Nette de l'unité de compte (C = A − B) | 3,64% |
| **Finale (A − E)** | **2,87%** |

---

## ODDO BHF SUST EURO CORP BD CR

`LU0145975222` · ODDO BHF ASSET MANAGEMENT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0145975222` |
| Société de gestion | ODDO BHF ASSET MANAGEMENT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,02% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,79%** |
| Taux de rétrocession de commissionnement | 0,55% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 3,39% |
| Nette de l'unité de compte (C = A − B) | 2,37% |
| **Finale (A − E)** | **1,60%** |

---

## SCHRODER EURO CORPORATE

`LU0113257694` · SCHRODER INVESTMENT MGT EUR SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0113257694` |
| Société de gestion | SCHRODER INVESTMENT MGT EUR SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,03% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,81%** |
| Taux de rétrocession de commissionnement | 0,38% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 4,47% |
| Nette de l'unité de compte (C = A − B) | 3,44% |
| **Finale (A − E)** | **2,66%** |
