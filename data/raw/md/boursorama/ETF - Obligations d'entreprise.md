---
grande_classe: "ETF"
type_actif: "Obligations d'entreprise"
slug: etf--obligations-d-entreprise
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 5
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU1681039647"
    nom: "AM EURO CORP SRI ETF 2 DR C"
    societe: "AMUNDI LUXEMBOURG SA"
    nature: "Compartiment SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "3,08%"
    frais_actif_b: "0,14%"
    perf_nette_c: "2,94%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "0,91%"
    perf_finale: "2,17%"
    retrocessions: "n.c."
  - isin: "IE00BG0J4B71"
    nom: "ISHARES BROAD H-Y CORP BD ETF"
    societe: "BLACKROCK ASSET MANAGEMENT IRL"
    nature: "SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,20%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "n.c."
  - isin: "IE00BZ163G84"
    nom: "VANGUARD EUR CORP BD ETF"
    societe: "VANGUARD GROUP IRELAND LIMITED"
    nature: "SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "-0,37%"
    frais_actif_b: "0,09%"
    perf_nette_c: "-0,46%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "0,84%"
    perf_finale: "-1,21%"
    retrocessions: "n.c."
  - isin: "IE00BZ036J45"
    nom: "XTRACKERS CORPOBD ETF 2D H"
    societe: "DWS INVESTMENT SA"
    nature: "SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,00%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "n.c."
  - isin: "LU0478205379"
    nom: "XTRACKERS II CORP BD ETF 1C"
    societe: "DB PLATINUM ADVISORS"
    nature: "SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,09%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "n.c."
---

# ETF — Obligations d'entreprise

5 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU1681039647` | AM EURO CORP SRI ETF 2 DR C | AMUNDI LUXEMBOURG SA | 0,91% | 2,17% | n.c. | non renseignée |
| `IE00BG0J4B71` | ISHARES BROAD H-Y CORP BD ETF | BLACKROCK ASSET MANAGEMENT IRL | n.c. | n.c. | n.c. | non renseignée |
| `IE00BZ163G84` | VANGUARD EUR CORP BD ETF | VANGUARD GROUP IRELAND LIMITED | 0,84% | -1,21% | n.c. | non renseignée |
| `IE00BZ036J45` | XTRACKERS CORPOBD ETF 2D H | DWS INVESTMENT SA | n.c. | n.c. | n.c. | non renseignée |
| `LU0478205379` | XTRACKERS II CORP BD ETF 1C | DB PLATINUM ADVISORS | n.c. | n.c. | n.c. | non renseignée |

---
## AM EURO CORP SRI ETF 2 DR C

`LU1681039647` · AMUNDI LUXEMBOURG SA · Compartiment SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `LU1681039647` |
| Société de gestion | AMUNDI LUXEMBOURG SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,14% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **0,91%** |
| Taux de rétrocession de commissionnement | n.c. |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 3,08% |
| Nette de l'unité de compte (C = A − B) | 2,94% |
| **Finale (A − E)** | **2,17%** |

---

## ISHARES BROAD H-Y CORP BD ETF

`IE00BG0J4B71` · BLACKROCK ASSET MANAGEMENT IRL · SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `IE00BG0J4B71` |
| Société de gestion | BLACKROCK ASSET MANAGEMENT IRL |
| Nature juridique | SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,20% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | n.c. |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## VANGUARD EUR CORP BD ETF

`IE00BZ163G84` · VANGUARD GROUP IRELAND LIMITED · SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `IE00BZ163G84` |
| Société de gestion | VANGUARD GROUP IRELAND LIMITED |
| Nature juridique | SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,09% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **0,84%** |
| Taux de rétrocession de commissionnement | n.c. |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -0,37% |
| Nette de l'unité de compte (C = A − B) | -0,46% |
| **Finale (A − E)** | **-1,21%** |

---

## XTRACKERS CORPOBD ETF 2D H

`IE00BZ036J45` · DWS INVESTMENT SA · SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `IE00BZ036J45` |
| Société de gestion | DWS INVESTMENT SA |
| Nature juridique | SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,00% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | n.c. |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## XTRACKERS II CORP BD ETF 1C

`LU0478205379` · DB PLATINUM ADVISORS · SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `LU0478205379` |
| Société de gestion | DB PLATINUM ADVISORS |
| Nature juridique | SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Obligations d'entreprise (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,09% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | n.c. |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |
