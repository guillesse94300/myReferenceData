---
grande_classe: "Actions"
type_actif: "Secteur Santé"
slug: actions--secteur-sante
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 9
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU0171307068"
    nom: "BGF WORLD HEALTHSCIENCE A2 EUR"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "1,87%"
    frais_actif_b: "1,81%"
    perf_nette_c: "0,06%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,56%"
    perf_finale: "-0,69%"
    retrocessions: "0,75%"
  - isin: "LU1822774284"
    nom: "BGF WORLD HEALTHSCIENCE A2 H"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "12,18%"
    frais_actif_b: "1,78%"
    perf_nette_c: "10,40%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,61%"
    perf_finale: "9,57%"
    retrocessions: "0,75%"
  - isin: "LU0108459040"
    nom: "CANDR EQUITIES L-BIOTECH-C"
    societe: "CANDRIAM SCA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "24,49%"
    frais_actif_b: "1,98%"
    perf_nette_c: "22,51%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,90%"
    perf_finale: "21,59%"
    retrocessions: "0,88%"
  - isin: "LU2036816820"
    nom: "CPR INVEST MEDTECH A"
    societe: "CPR ASSET MANAGEMENT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "-5,59%"
    frais_actif_b: "1,98%"
    perf_nette_c: "-7,57%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,67%"
    perf_finale: "-8,27%"
    retrocessions: "0,75%"
  - isin: "LU1160356009"
    nom: "EDR F HEALTHCARE A EUR"
    societe: "EDR ASSET MANAGEMENT LUX SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "2,63%"
    frais_actif_b: "2,18%"
    perf_nette_c: "0,45%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,93%"
    perf_finale: "-0,30%"
    retrocessions: "1,00%"
  - isin: "LU0261952419"
    nom: "FF GLOBAL HEALTH CARE A A EUR"
    societe: "FIL INVESTMENT MANAGEMENT LUX"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "-5,36%"
    frais_actif_b: "1,90%"
    perf_nette_c: "-7,26%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,60%"
    perf_finale: "-7,96%"
    retrocessions: "0,75%"
  - isin: "LU0880062913"
    nom: "JPM GLOB HEALTHCARE FND A C"
    societe: "JPMORGAN ASSET MANAGEMENT EUR"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "2,75%"
    frais_actif_b: "1,70%"
    perf_nette_c: "1,05%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,46%"
    perf_finale: "0,30%"
    retrocessions: "0,75%"
  - isin: "LU2932856839"
    nom: "MIROVA THEM HEALTH RA USD C"
    societe: "NATIXIS INVEST MANAGERS INTL"
    nature: "SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "2,01%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,88%"
  - isin: "LU0190161025"
    nom: "PICTET BIOTECH HP EUR"
    societe: "PICTET ASSET MANAGEMENT EUROPE"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "48,70%"
    frais_actif_b: "2,04%"
    perf_nette_c: "46,66%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "3,14%"
    perf_finale: "45,57%"
    retrocessions: "0,80%"
---

# Actions — Secteur Santé

9 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU0171307068` | BGF WORLD HEALTHSCIENCE A2 EUR | BLACKROCK (LUXEMBOURG) SA | 2,56% | -0,69% | 0,75% | Article 8 |
| `LU1822774284` | BGF WORLD HEALTHSCIENCE A2 H | BLACKROCK (LUXEMBOURG) SA | 2,61% | 9,57% | 0,75% | Article 8 |
| `LU0108459040` | CANDR EQUITIES L-BIOTECH-C | CANDRIAM SCA | 2,90% | 21,59% | 0,88% | Article 8 |
| `LU2036816820` | CPR INVEST MEDTECH A | CPR ASSET MANAGEMENT | 2,67% | -8,27% | 0,75% | Article 8 |
| `LU1160356009` | EDR F HEALTHCARE A EUR | EDR ASSET MANAGEMENT LUX SA | 2,93% | -0,30% | 1,00% | Article 8 |
| `LU0261952419` | FF GLOBAL HEALTH CARE A A EUR | FIL INVESTMENT MANAGEMENT LUX | 2,60% | -7,96% | 0,75% | Article 8 |
| `LU0880062913` | JPM GLOB HEALTHCARE FND A C | JPMORGAN ASSET MANAGEMENT EUR | 2,46% | 0,30% | 0,75% | Article 8 |
| `LU2932856839` | MIROVA THEM HEALTH RA USD C | NATIXIS INVEST MANAGERS INTL | n.c. | n.c. | 0,88% | Article 9 |
| `LU0190161025` | PICTET BIOTECH HP EUR | PICTET ASSET MANAGEMENT EUROPE | 3,14% | 45,57% | 0,80% | Article 9 |

---
## BGF WORLD HEALTHSCIENCE A2 EUR

`LU0171307068` · BLACKROCK (LUXEMBOURG) SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0171307068` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,81% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,56%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 1,87% |
| Nette de l'unité de compte (C = A − B) | 0,06% |
| **Finale (A − E)** | **-0,69%** |

---

## BGF WORLD HEALTHSCIENCE A2 H

`LU1822774284` · BLACKROCK (LUXEMBOURG) SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1822774284` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,78% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,61%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 12,18% |
| Nette de l'unité de compte (C = A − B) | 10,40% |
| **Finale (A − E)** | **9,57%** |

---

## CANDR EQUITIES L-BIOTECH-C

`LU0108459040` · CANDRIAM SCA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0108459040` |
| Société de gestion | CANDRIAM SCA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,98% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,90%** |
| Taux de rétrocession de commissionnement | 0,88% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 24,49% |
| Nette de l'unité de compte (C = A − B) | 22,51% |
| **Finale (A − E)** | **21,59%** |

---

## CPR INVEST MEDTECH A

`LU2036816820` · CPR ASSET MANAGEMENT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU2036816820` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,98% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,67%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -5,59% |
| Nette de l'unité de compte (C = A − B) | -7,57% |
| **Finale (A − E)** | **-8,27%** |

---

## EDR F HEALTHCARE A EUR

`LU1160356009` · EDR ASSET MANAGEMENT LUX SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1160356009` |
| Société de gestion | EDR ASSET MANAGEMENT LUX SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,18% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,93%** |
| Taux de rétrocession de commissionnement | 1,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 2,63% |
| Nette de l'unité de compte (C = A − B) | 0,45% |
| **Finale (A − E)** | **-0,30%** |

---

## FF GLOBAL HEALTH CARE A A EUR

`LU0261952419` · FIL INVESTMENT MANAGEMENT LUX · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0261952419` |
| Société de gestion | FIL INVESTMENT MANAGEMENT LUX |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,90% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,60%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | -5,36% |
| Nette de l'unité de compte (C = A − B) | -7,26% |
| **Finale (A − E)** | **-7,96%** |

---

## JPM GLOB HEALTHCARE FND A C

`LU0880062913` · JPMORGAN ASSET MANAGEMENT EUR · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0880062913` |
| Société de gestion | JPMORGAN ASSET MANAGEMENT EUR |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,70% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,46%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 2,75% |
| Nette de l'unité de compte (C = A − B) | 1,05% |
| **Finale (A − E)** | **0,30%** |

---

## MIROVA THEM HEALTH RA USD C

`LU2932856839` · NATIXIS INVEST MANAGERS INTL · SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU2932856839` |
| Société de gestion | NATIXIS INVEST MANAGERS INTL |
| Nature juridique | SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,01% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,88% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## PICTET BIOTECH HP EUR

`LU0190161025` · PICTET ASSET MANAGEMENT EUROPE · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU0190161025` |
| Société de gestion | PICTET ASSET MANAGEMENT EUROPE |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Santé (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,04% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **3,14%** |
| Taux de rétrocession de commissionnement | 0,80% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 48,70% |
| Nette de l'unité de compte (C = A − B) | 46,66% |
| **Finale (A − E)** | **45,57%** |
