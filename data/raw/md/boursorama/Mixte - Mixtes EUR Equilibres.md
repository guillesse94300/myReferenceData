---
grande_classe: "Mixte"
type_actif: "Mixtes EUR Equilibrés"
slug: mixte--mixtes-eur-equilibres
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 4
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "LU0212925753"
    nom: "BGF GLOBAL ALLOC FD H A2 EUR"
    societe: "BLACKROCK (LUXEMBOURG) SA"
    nature: "Compartiment SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "16,40%"
    frais_actif_b: "1,78%"
    perf_nette_c: "14,62%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,64%"
    perf_finale: "13,77%"
    retrocessions: "0,75%"
  - isin: "FR0010149203"
    nom: "CARMIGNAC MULTI EXPERTISE"
    societe: "CARMIGNAC GESTION"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "6,34%"
    frais_actif_b: "1,60%"
    perf_nette_c: "4,74%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,39%"
    perf_finale: "3,96%"
    retrocessions: "1,00%"
  - isin: "FR0010135103"
    nom: "CARMIGNAC PATRIMOINE A EUR ACC"
    societe: "CARMIGNAC GESTION"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "13,92%"
    frais_actif_b: "1,80%"
    perf_nette_c: "12,12%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,64%"
    perf_finale: "11,28%"
    retrocessions: "0,75%"
  - isin: "LU0740858229"
    nom: "JPM GLOBAL INC FND A EUR CAP"
    societe: "JPMORGAN ASSET MANAGEMENT EUR"
    nature: "Compartiment SICAV"
    sfdr: "non renseignée"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "9,94%"
    frais_actif_b: "1,40%"
    perf_nette_c: "8,54%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,21%"
    perf_finale: "7,73%"
    retrocessions: "0,62%"
---

# Mixte — Mixtes EUR Equilibrés

4 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `LU0212925753` | BGF GLOBAL ALLOC FD H A2 EUR | BLACKROCK (LUXEMBOURG) SA | 2,64% | 13,77% | 0,75% | non renseignée |
| `FR0010149203` | CARMIGNAC MULTI EXPERTISE | CARMIGNAC GESTION | 2,39% | 3,96% | 1,00% | Article 8 |
| `FR0010135103` | CARMIGNAC PATRIMOINE A EUR ACC | CARMIGNAC GESTION | 2,64% | 11,28% | 0,75% | Article 8 |
| `LU0740858229` | JPM GLOBAL INC FND A EUR CAP | JPMORGAN ASSET MANAGEMENT EUR | 2,21% | 7,73% | 0,62% | non renseignée |

---
## BGF GLOBAL ALLOC FD H A2 EUR

`LU0212925753` · BLACKROCK (LUXEMBOURG) SA · Compartiment SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `LU0212925753` |
| Société de gestion | BLACKROCK (LUXEMBOURG) SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Equilibrés (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,78% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,64%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 16,40% |
| Nette de l'unité de compte (C = A − B) | 14,62% |
| **Finale (A − E)** | **13,77%** |

---

## CARMIGNAC MULTI EXPERTISE

`FR0010149203` · CARMIGNAC GESTION · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010149203` |
| Société de gestion | CARMIGNAC GESTION |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Equilibrés (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,60% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,39%** |
| Taux de rétrocession de commissionnement | 1,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 6,34% |
| Nette de l'unité de compte (C = A − B) | 4,74% |
| **Finale (A − E)** | **3,96%** |

---

## CARMIGNAC PATRIMOINE A EUR ACC

`FR0010135103` · CARMIGNAC GESTION · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010135103` |
| Société de gestion | CARMIGNAC GESTION |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Equilibrés (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,80% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,64%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 13,92% |
| Nette de l'unité de compte (C = A − B) | 12,12% |
| **Finale (A − E)** | **11,28%** |

---

## JPM GLOBAL INC FND A EUR CAP

`LU0740858229` · JPMORGAN ASSET MANAGEMENT EUR · Compartiment SICAV · SFDR non renseignée

### Identité

| | |
|---|---|
| Code ISIN | `LU0740858229` |
| Société de gestion | JPMORGAN ASSET MANAGEMENT EUR |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | non renseignée |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Equilibrés (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,40% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,21%** |
| Taux de rétrocession de commissionnement | 0,62% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 9,94% |
| Nette de l'unité de compte (C = A − B) | 8,54% |
| **Finale (A − E)** | **7,73%** |
