---
grande_classe: "Mixte"
type_actif: "Mixtes EUR Prudents"
slug: mixte--mixtes-eur-prudents
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 7
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "FR0010177899"
    nom: "CHOIX SOLIDAIRE C"
    societe: "ECOFI INVESTISSEMENTS"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "8,72%"
    frais_actif_b: "0,94%"
    perf_nette_c: "7,78%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "1,75%"
    perf_finale: "6,97%"
    retrocessions: "0,40%"
  - isin: "LU0284394235"
    nom: "DNCA INVEST EUROSE A EUR"
    societe: "DNCA FINANCE"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "9,30%"
    frais_actif_b: "1,49%"
    perf_nette_c: "7,81%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,30%"
    perf_finale: "7,00%"
    retrocessions: "0,63%"
  - isin: "LU0992632538"
    nom: "EDR INCOME EUROPE"
    societe: "EDMOND ROTHSCHILD ASSET MNGT"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "7,28%"
    frais_actif_b: "1,64%"
    perf_nette_c: "5,64%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,43%"
    perf_finale: "4,85%"
    retrocessions: "0,65%"
  - isin: "FR0012355139"
    nom: "LAZARD PATRIMOINE R CAP"
    societe: "LAZARD FRERES GESTION SAS"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "2,60%"
    frais_actif_b: "1,54%"
    perf_nette_c: "1,06%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,30%"
    perf_finale: "0,31%"
    retrocessions: "0,69%"
  - isin: "FR0010308825"
    nom: "SWISS LIFE FUNDS F DEFENSIVE P"
    societe: "SWISS LIFE ASSET MANAGERS FRA"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "7,10%"
    frais_actif_b: "1,80%"
    perf_nette_c: "5,30%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,59%"
    perf_finale: "4,51%"
    retrocessions: "0,60%"
  - isin: "FR0007078589"
    nom: "SYCOMORE ALLOC PATRIMOINE R"
    societe: "SYCOMORE ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "7,44%"
    frais_actif_b: "2,08%"
    perf_nette_c: "5,36%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,87%"
    perf_finale: "4,57%"
    retrocessions: "0,90%"
  - isin: "FR0007072160"
    nom: "TRUSTEAM OPTIMUM R"
    societe: "AURIS GESTION"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "annexe IA SwissLife (par ISIN)"
    perf_brute_a: "4,95%"
    frais_actif_b: "1,40%"
    perf_nette_c: "3,56%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,18%"
    perf_finale: "2,78%"
    retrocessions: "0,60%"
---

# Mixte — Mixtes EUR Prudents

7 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `FR0010177899` | CHOIX SOLIDAIRE C | ECOFI INVESTISSEMENTS | 1,75% | 6,97% | 0,40% | Article 8 |
| `LU0284394235` | DNCA INVEST EUROSE A EUR | DNCA FINANCE | 2,30% | 7,00% | 0,63% | Article 8 |
| `LU0992632538` | EDR INCOME EUROPE | EDMOND ROTHSCHILD ASSET MNGT | 2,43% | 4,85% | 0,65% | Article 8 |
| `FR0012355139` | LAZARD PATRIMOINE R CAP | LAZARD FRERES GESTION SAS | 2,30% | 0,31% | 0,69% | Article 8 |
| `FR0010308825` | SWISS LIFE FUNDS F DEFENSIVE P | SWISS LIFE ASSET MANAGERS FRA | 2,59% | 4,51% | 0,60% | Article 8 |
| `FR0007078589` | SYCOMORE ALLOC PATRIMOINE R | SYCOMORE ASSET MANAGEMENT | 2,87% | 4,57% | 0,90% | Article 8 |
| `FR0007072160` | TRUSTEAM OPTIMUM R | AURIS GESTION | 2,18% | 2,78% | 0,60% | Article 8 |

---
## CHOIX SOLIDAIRE C

`FR0010177899` · ECOFI INVESTISSEMENTS · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010177899` |
| Société de gestion | ECOFI INVESTISSEMENTS |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,94% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **1,75%** |
| Taux de rétrocession de commissionnement | 0,40% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 8,72% |
| Nette de l'unité de compte (C = A − B) | 7,78% |
| **Finale (A − E)** | **6,97%** |

---

## DNCA INVEST EUROSE A EUR

`LU0284394235` · DNCA FINANCE · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0284394235` |
| Société de gestion | DNCA FINANCE |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,49% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,30%** |
| Taux de rétrocession de commissionnement | 0,63% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 9,30% |
| Nette de l'unité de compte (C = A − B) | 7,81% |
| **Finale (A − E)** | **7,00%** |

---

## EDR INCOME EUROPE

`LU0992632538` · EDMOND ROTHSCHILD ASSET MNGT · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0992632538` |
| Société de gestion | EDMOND ROTHSCHILD ASSET MNGT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,64% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,43%** |
| Taux de rétrocession de commissionnement | 0,65% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 7,28% |
| Nette de l'unité de compte (C = A − B) | 5,64% |
| **Finale (A − E)** | **4,85%** |

---

## LAZARD PATRIMOINE R CAP

`FR0012355139` · LAZARD FRERES GESTION SAS · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0012355139` |
| Société de gestion | LAZARD FRERES GESTION SAS |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,54% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,30%** |
| Taux de rétrocession de commissionnement | 0,69% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 2,60% |
| Nette de l'unité de compte (C = A − B) | 1,06% |
| **Finale (A − E)** | **0,31%** |

---

## SWISS LIFE FUNDS F DEFENSIVE P

`FR0010308825` · SWISS LIFE ASSET MANAGERS FRA · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0010308825` |
| Société de gestion | SWISS LIFE ASSET MANAGERS FRA |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,80% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,59%** |
| Taux de rétrocession de commissionnement | 0,60% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 7,10% |
| Nette de l'unité de compte (C = A − B) | 5,30% |
| **Finale (A − E)** | **4,51%** |

---

## SYCOMORE ALLOC PATRIMOINE R

`FR0007078589` · SYCOMORE ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0007078589` |
| Société de gestion | SYCOMORE ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,08% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,87%** |
| Taux de rétrocession de commissionnement | 0,90% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 7,44% |
| Nette de l'unité de compte (C = A − B) | 5,36% |
| **Finale (A − E)** | **4,57%** |

---

## TRUSTEAM OPTIMUM R

`FR0007072160` · AURIS GESTION · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0007072160` |
| Société de gestion | AURIS GESTION |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Mixtes EUR Prudents (annexe IA SwissLife (par ISIN)) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,40% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,18%** |
| Taux de rétrocession de commissionnement | 0,60% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 4,95% |
| Nette de l'unité de compte (C = A − B) | 3,56% |
| **Finale (A − E)** | **2,78%** |
