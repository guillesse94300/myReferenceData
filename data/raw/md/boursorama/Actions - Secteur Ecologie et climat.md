---
grande_classe: "Actions"
type_actif: "Secteur Écologie & climat"
slug: actions--secteur-ecologie-climat
contrat: "Bourso Vie (5101) — gestion libre"
assureur: "Generali Vie"
millesime: "2026-09-18"
nombre_de_supports: 12
source: "Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026"
supports:
  - isin: "FR0000991424"
    nom: "AMUNDI ACT EUR ENVIRONNEMENT P"
    societe: "AMUNDI ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "14,04%"
    frais_actif_b: "1,75%"
    perf_nette_c: "12,29%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,59%"
    perf_finale: "11,45%"
    retrocessions: "0,59%"
  - isin: "FR0011199371"
    nom: "AMUNDI EQUILIBRE CLIMAT C"
    societe: "AMUNDI ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "8,52%"
    frais_actif_b: "1,83%"
    perf_nette_c: "6,69%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,63%"
    perf_finale: "5,89%"
    retrocessions: "0,65%"
  - isin: "LU1883868819"
    nom: "AMUNDI EUROPE EQ CLIMATE A C"
    societe: "AMUNDI LUXEMBOURG SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "13,25%"
    frais_actif_b: "1,78%"
    perf_nette_c: "11,47%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,62%"
    perf_finale: "10,63%"
    retrocessions: "0,75%"
  - isin: "FR0013216207"
    nom: "AMUNDI KBI AQUA C"
    societe: "AMUNDI ASSET MANAGEMENT"
    nature: "FCP"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "0,83%"
    frais_actif_b: "1,90%"
    perf_nette_c: "-1,07%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,64%"
    perf_finale: "-1,81%"
    retrocessions: "0,74%"
  - isin: "LU1165135440"
    nom: "BNP PARIBAS FUNDS AQUA"
    societe: "BNP PARIBAS ASSET MNGT LUX SA"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "1,67%"
    frais_actif_b: "2,21%"
    perf_nette_c: "-0,54%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,96%"
    perf_finale: "-1,29%"
    retrocessions: "0,96%"
  - isin: "FR001400RWM2"
    nom: "BOURSO CLIMAT"
    societe: "SG INVT SOLUTIONS FRANCE"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "n.c."
    frais_actif_b: "0,59%"
    perf_nette_c: "n.c."
    frais_contrat_d: "0,75%"
    frais_totaux_e: "n.c."
    perf_finale: "n.c."
    retrocessions: "0,00%"
  - isin: "LU2389405080"
    nom: "CPR INVEST HYDROGEN A"
    societe: "CPR ASSET MANAGEMENT"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "17,90%"
    frais_actif_b: "1,96%"
    perf_nette_c: "15,94%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,83%"
    perf_finale: "15,07%"
    retrocessions: "0,80%"
  - isin: "LU1902443420"
    nom: "CPR INVEST-CLIMATE ACT -A-ACC"
    societe: "CPR ASSET MANAGEMENT"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "5,35%"
    frais_actif_b: "1,95%"
    perf_nette_c: "3,40%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,73%"
    perf_finale: "2,62%"
    retrocessions: "0,78%"
  - isin: "FR0000982449"
    nom: "HSBC EURO EQ GREEN TRANSIT AC"
    societe: "HSBC GLOBAL ASSET MNGMT FRANCE"
    nature: "FCP"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "13,87%"
    frais_actif_b: "1,60%"
    perf_nette_c: "12,27%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,44%"
    perf_finale: "11,43%"
    retrocessions: "0,75%"
  - isin: "LU0914731947"
    nom: "MIROVA EURO SUSTAINABLE EQ R/A"
    societe: "NATIXIS INVEST MANAGERS INTL"
    nature: "Compartiment SICAV"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "13,94%"
    frais_actif_b: "1,86%"
    perf_nette_c: "12,08%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,70%"
    perf_finale: "11,24%"
    retrocessions: "0,78%"
  - isin: "LU0302446645"
    nom: "SCHRODER ISF GLB CLMT CHG EQ A"
    societe: "SCHRODER INVESTMENT MGT EUR SA"
    nature: "SICAV"
    sfdr: "Article 8"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "11,04%"
    frais_actif_b: "1,83%"
    perf_nette_c: "9,21%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,65%"
    perf_finale: "8,39%"
    retrocessions: "0,75%"
  - isin: "FR0000444275"
    nom: "SG AM ACT EUR ENVIRON P CAP"
    societe: "SOCIETE GENERALE GESTION"
    nature: "FCP"
    sfdr: "Article 9"
    avenant: "non"
    origine_classification: "déduite du libellé"
    perf_brute_a: "15,11%"
    frais_actif_b: "1,94%"
    perf_nette_c: "13,17%"
    frais_contrat_d: "0,75%"
    frais_totaux_e: "2,79%"
    perf_finale: "12,32%"
    retrocessions: "0,82%"
---

# Actions — Secteur Écologie & climat

12 support(s) · Bourso Vie (5101) — Annexe financière, liste des supports en gestion libre, Generali Vie / BoursoBank, 18 septembre 2026.

Frais de gestion du contrat : 0,75 %. Les frais totaux suivent la formule de l'assureur
**E = B + (1 + C) × D**, qui fait croître les frais affichés avec la performance :
elle n'est pas directement comparable à une somme de taux.
La performance porte sur le dernier exercice clos.

## Tableau de synthèse

| ISIN | Support | Société | Frais totaux | Perf. finale | Rétrocessions | SFDR |
|---|---|---|---|---|---|---|
| `FR0000991424` | AMUNDI ACT EUR ENVIRONNEMENT P | AMUNDI ASSET MANAGEMENT | 2,59% | 11,45% | 0,59% | Article 9 |
| `FR0011199371` | AMUNDI EQUILIBRE CLIMAT C | AMUNDI ASSET MANAGEMENT | 2,63% | 5,89% | 0,65% | Article 8 |
| `LU1883868819` | AMUNDI EUROPE EQ CLIMATE A C | AMUNDI LUXEMBOURG SA | 2,62% | 10,63% | 0,75% | Article 8 |
| `FR0013216207` | AMUNDI KBI AQUA C | AMUNDI ASSET MANAGEMENT | 2,64% | -1,81% | 0,74% | Article 8 |
| `LU1165135440` | BNP PARIBAS FUNDS AQUA | BNP PARIBAS ASSET MNGT LUX SA | 2,96% | -1,29% | 0,96% | Article 9 |
| `FR001400RWM2` | BOURSO CLIMAT | SG INVT SOLUTIONS FRANCE | n.c. | n.c. | 0,00% | Article 8 |
| `LU2389405080` | CPR INVEST HYDROGEN A | CPR ASSET MANAGEMENT | 2,83% | 15,07% | 0,80% | Article 8 |
| `LU1902443420` | CPR INVEST-CLIMATE ACT -A-ACC | CPR ASSET MANAGEMENT | 2,73% | 2,62% | 0,78% | Article 9 |
| `FR0000982449` | HSBC EURO EQ GREEN TRANSIT AC | HSBC GLOBAL ASSET MNGMT FRANCE | 2,44% | 11,43% | 0,75% | Article 9 |
| `LU0914731947` | MIROVA EURO SUSTAINABLE EQ R/A | NATIXIS INVEST MANAGERS INTL | 2,70% | 11,24% | 0,78% | Article 9 |
| `LU0302446645` | SCHRODER ISF GLB CLMT CHG EQ A | SCHRODER INVESTMENT MGT EUR SA | 2,65% | 8,39% | 0,75% | Article 8 |
| `FR0000444275` | SG AM ACT EUR ENVIRON P CAP | SOCIETE GENERALE GESTION | 2,79% | 12,32% | 0,82% | Article 9 |

---
## AMUNDI ACT EUR ENVIRONNEMENT P

`FR0000991424` · AMUNDI ASSET MANAGEMENT · FCP · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `FR0000991424` |
| Société de gestion | AMUNDI ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,75% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,59%** |
| Taux de rétrocession de commissionnement | 0,59% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 14,04% |
| Nette de l'unité de compte (C = A − B) | 12,29% |
| **Finale (A − E)** | **11,45%** |

---

## AMUNDI EQUILIBRE CLIMAT C

`FR0011199371` · AMUNDI ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0011199371` |
| Société de gestion | AMUNDI ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,83% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,63%** |
| Taux de rétrocession de commissionnement | 0,65% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 8,52% |
| Nette de l'unité de compte (C = A − B) | 6,69% |
| **Finale (A − E)** | **5,89%** |

---

## AMUNDI EUROPE EQ CLIMATE A C

`LU1883868819` · AMUNDI LUXEMBOURG SA · Compartiment SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU1883868819` |
| Société de gestion | AMUNDI LUXEMBOURG SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,78% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,62%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 13,25% |
| Nette de l'unité de compte (C = A − B) | 11,47% |
| **Finale (A − E)** | **10,63%** |

---

## AMUNDI KBI AQUA C

`FR0013216207` · AMUNDI ASSET MANAGEMENT · FCP · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR0013216207` |
| Société de gestion | AMUNDI ASSET MANAGEMENT |
| Nature juridique | FCP |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,90% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,64%** |
| Taux de rétrocession de commissionnement | 0,74% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 0,83% |
| Nette de l'unité de compte (C = A − B) | -1,07% |
| **Finale (A − E)** | **-1,81%** |

---

## BNP PARIBAS FUNDS AQUA

`LU1165135440` · BNP PARIBAS ASSET MNGT LUX SA · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU1165135440` |
| Société de gestion | BNP PARIBAS ASSET MNGT LUX SA |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 2,21% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,96%** |
| Taux de rétrocession de commissionnement | 0,96% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 1,67% |
| Nette de l'unité de compte (C = A − B) | -0,54% |
| **Finale (A − E)** | **-1,29%** |

---

## BOURSO CLIMAT

`FR001400RWM2` · SG INVT SOLUTIONS FRANCE · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `FR001400RWM2` |
| Société de gestion | SG INVT SOLUTIONS FRANCE |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 0,59% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **n.c.** |
| Taux de rétrocession de commissionnement | 0,00% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | n.c. |
| Nette de l'unité de compte (C = A − B) | n.c. |
| **Finale (A − E)** | **n.c.** |

---

## CPR INVEST HYDROGEN A

`LU2389405080` · CPR ASSET MANAGEMENT · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU2389405080` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,96% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,83%** |
| Taux de rétrocession de commissionnement | 0,80% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 17,90% |
| Nette de l'unité de compte (C = A − B) | 15,94% |
| **Finale (A − E)** | **15,07%** |

---

## CPR INVEST-CLIMATE ACT -A-ACC

`LU1902443420` · CPR ASSET MANAGEMENT · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU1902443420` |
| Société de gestion | CPR ASSET MANAGEMENT |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,95% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,73%** |
| Taux de rétrocession de commissionnement | 0,78% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 5,35% |
| Nette de l'unité de compte (C = A − B) | 3,40% |
| **Finale (A − E)** | **2,62%** |

---

## HSBC EURO EQ GREEN TRANSIT AC

`FR0000982449` · HSBC GLOBAL ASSET MNGMT FRANCE · FCP · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `FR0000982449` |
| Société de gestion | HSBC GLOBAL ASSET MNGMT FRANCE |
| Nature juridique | FCP |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,60% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,44%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 13,87% |
| Nette de l'unité de compte (C = A − B) | 12,27% |
| **Finale (A − E)** | **11,43%** |

---

## MIROVA EURO SUSTAINABLE EQ R/A

`LU0914731947` · NATIXIS INVEST MANAGERS INTL · Compartiment SICAV · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `LU0914731947` |
| Société de gestion | NATIXIS INVEST MANAGERS INTL |
| Nature juridique | Compartiment SICAV |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,86% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,70%** |
| Taux de rétrocession de commissionnement | 0,78% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 13,94% |
| Nette de l'unité de compte (C = A − B) | 12,08% |
| **Finale (A − E)** | **11,24%** |

---

## SCHRODER ISF GLB CLMT CHG EQ A

`LU0302446645` · SCHRODER INVESTMENT MGT EUR SA · SICAV · SFDR Article 8

### Identité

| | |
|---|---|
| Code ISIN | `LU0302446645` |
| Société de gestion | SCHRODER INVESTMENT MGT EUR SA |
| Nature juridique | SICAV |
| Classification SFDR | Article 8 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,83% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,65%** |
| Taux de rétrocession de commissionnement | 0,75% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 11,04% |
| Nette de l'unité de compte (C = A − B) | 9,21% |
| **Finale (A − E)** | **8,39%** |

---

## SG AM ACT EUR ENVIRON P CAP

`FR0000444275` · SOCIETE GENERALE GESTION · FCP · SFDR Article 9

### Identité

| | |
|---|---|
| Code ISIN | `FR0000444275` |
| Société de gestion | SOCIETE GENERALE GESTION |
| Nature juridique | FCP |
| Classification SFDR | Article 9 |
| Avenant spécifique | non |
| Classification d'actif | Secteur Écologie & climat (déduite du libellé) |

### Frais

| Poste | Taux |
|---|---|
| Frais de gestion de l'actif (B) | 1,94% |
| Frais de gestion du contrat (D) | 0,75% |
| **Frais totaux (E = B + (1 + C) × D)** | **2,79%** |
| Taux de rétrocession de commissionnement | 0,82% |

### Performances du dernier exercice clos

| Niveau | Taux |
|---|---|
| Brute de l'actif (A) | 15,11% |
| Nette de l'unité de compte (C = A − B) | 13,17% |
| **Finale (A − E)** | **12,32%** |
